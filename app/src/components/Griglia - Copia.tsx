...
import { C, coloreDiSfondo, FONT, ombra, RAGGIO } from '../theme';
...
function Cella({ cella, lato, indiceColonna }: { cella: StatoCella; lato: number; indiceColonna: number }) {
  ...
  // Cella valutata: tinta piena (piatta, come il riferimento) + flip di rivelazione.
  if (cella.tipo === 'valutata') {
  ...
  const bordo =
    cella.tipo === 'inserita' ? C.bordoAttivo : cella.attiva ? C.bordoAttivoSoft : C.bordoVuoto;
  return (
    <Animated.View style={[styles.cella, styles.cellaVuota, dim, { borderColor: bordo, transform: [{ scale: pop }] }]}>
...
const styles = StyleSheet.create({
  ...
  cellaVuota: { borderWidth: 2, backgroundColor: 'rgba(255,255,255,0.02)' },
